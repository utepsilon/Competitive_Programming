        ...
        if gf == 1:
            ...
            if x.isdigit():
                if x in dates.keys():
                    ...
                    dates[x] = 1;
    gf = 0
x = [k for k, v in dates.items() if v == max(dates.values())] #to check if there exist multiple max or none max 
max1 = 0
#print(x)
if len(x)>1 or len(x)<1 :
    max1 = -2
else:              
    max1 =  max(dates, key= lambda x: dates[x]) 

if max1=='19' or max1=='20':
    print("Date")
else:
    print("No Date")
